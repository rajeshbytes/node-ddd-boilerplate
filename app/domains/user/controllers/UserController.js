...
const UserRequest = require("@domains/user/validations/UserRequest");
...
class UserController {
  ...
  static async store(req, res) {
    try {
      ...
      res
        .status(201)
        .json({ message: "User created", status: true, id: result.id });
    } catch (error) {
      if (error.name === "ZodError") {
        const issues = error.issues || [];
        return res.status(422).json({
          status: false,
          message: "Validation failed or missing field",
          errors: issues.map((e) => ({
            field: e.path.join("."),
            message: e.message,
          })),
